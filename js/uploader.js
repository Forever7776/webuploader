(function( $ ){
        ...
        var selector = options.selector || '#uploader';
        var url = options.url;
        var fileNumLimit = options.limit || 1;
        var defaultImageUrl = options.defaultImageUrl || '';
        var formData = options.formData || '';
        var accept = options.accept || {title: 'Files', extensions: 'gif,jpg,jpeg,bmp,png,mp3,amr', mimeTypes: 'image/*,audio/*'};
        var fileSizeLimit = options.fileSizeLimit || 100 * 1024 * 1024;
        var fileSingleSizeLimit = options.fileSingleSizeLimit || 5 * 1024 * 1024;
        ...
        uploader.onUploadSuccess = function (file, response) {
            if (response.http_status_code == 200 ) {
                $('#' + file.id).find('.success').append('<input type="hidden" name="'+$(selector).data('name')+'" value="'+response.image.url+'"/> ');
            }else{
